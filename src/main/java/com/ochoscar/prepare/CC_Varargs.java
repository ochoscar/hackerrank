...
class Add {
    ...
    public void add(Integer... data) {
        Integer sum = Arrays.stream(data).reduce((acc,item) -> acc + item).get();

        String sumString = Arrays.stream(data).map(item -> item.toString()).collect(Collectors.joining("+"));
        System.out.println(sumString + "=" + sum);
    }

}
public class CC_Varargs {
