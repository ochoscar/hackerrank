...
public class BG_JavaList {
    ...
    public static void main(String[] args) {
        ...
        List<Integer> lista = Arrays.stream(strNumbers.split(" ")).map(Integer::new).collect(Collectors.toList());
        int q = sc.nextInt();
        sc.nextLine();
        ...
        for(int i = 0; i < q; i++) {
            ...
            if(op.equalsIgnoreCase("Insert")) {
                String[] queryNums = query.split(" ");
                int x = Integer.parseInt(queryNums[0]);
                Integer y = Integer.parseInt(queryNums[1]);
                lista.add(x, y);
            } else if(op.equalsIgnoreCase("Delete")) {
                int d = Integer.parseInt(query);
                lista.remove(d);
            }
        }
